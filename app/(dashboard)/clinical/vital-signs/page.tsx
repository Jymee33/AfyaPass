  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-slate-900">Vital Signs Records</h1>
      ...
      <Card className="p-0 overflow-hidden">
        <div className="overflow-x-auto">
          <Table>
            <thead>
              <tr>
                <th className="text-left py-3 px-4 font-medium text-sm text-slate-500">Recorded Date</th>
                <th className="text-left py-3 px-4 font-medium text-sm text-slate-500">Patient</th>
                <th className="text-left py-3 px-4 font-medium text-sm text-slate-500">Blood Pressure</th>
                <th className="text-left py-3 px-4 font-medium text-sm text-slate-500">Heart Rate</th>
                <th className="text-left py-3 px-4 font-medium text-sm text-slate-500">Temperature</th>
                <th className="text-left py-3 px-4 font-medium text-sm text-slate-500">SpO2</th>
            ...
            <tbody>
              ...
              {mockVitalSigns.map((vital) => {
                ...
                return (
                  <tr key={vital.id} className="border-t border-slate-100 hover:bg-slate-50">
                    <td className="py-3 px-4 text-sm text-slate-600 whitespace-nowrap">
                      {new Date(vital.recordedAt).toLocaleString()}
                    </td>
                    <td className="py-3 px-4">
                      <div className="text-sm text-slate-900 font-medium">{vital.patientId.substring(0,8)}...</div>
                    </td>
                    <td className="py-3 px-4">
                    ...
                    <td className="py-3 px-4">
                      <div className="text-sm text-slate-900">Dr. {vital.recordedBy.substring(0,6)}...</div>
                    </td>
                  </tr>
                );
              })}
