  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h1 className="text-2xl font-semibold text-gray-900">Clinical Encounters</h1>
          <p className="text-sm text-gray-500 mt-1">Manage and view patient visits and encounters</p>
        </div>
        <Button>
      ...
      <Card className="p-0 overflow-hidden">
        <div className="overflow-x-auto">
          <Table>
            <thead>
              <tr>
                <th className="text-left py-3 px-4 font-medium text-sm text-gray-500">Date/Time</th>
                <th className="text-left py-3 px-4 font-medium text-sm text-gray-500">Patient</th>
                <th className="text-left py-3 px-4 font-medium text-sm text-gray-500">Facility & Provider</th>
                <th className="text-left py-3 px-4 font-medium text-sm text-gray-500">Type</th>
                <th className="text-left py-3 px-4 font-medium text-sm text-gray-500">Primary Diagnosis</th>
            ...
            <tbody>
              ...
              {mockEncounters.map((encounter) => (
                <tr key={encounter.id} className="border-t border-gray-100 hover:bg-gray-50">
                  <td className="py-3 px-4 text-sm text-gray-600 whitespace-nowrap">
                    {new Date(encounter.encounterDate).toLocaleString()}
                  </td>
                  <td className="py-3 px-4">
                    <div className="font-medium text-sm text-gray-900">{encounter.patientId.substring(0,8)}...</div>
                    <div className="text-xs text-gray-500">{encounter.patientId}</div>
                  </td>
                  <td className="py-3 px-4">
                    <div className="text-sm text-gray-900">{encounter.facilityId}</div>
                    <div className="text-xs text-gray-500">Dr. {encounter.workerId.substring(0,6)}...</div>
                  </td>
                  <td className="py-3 px-4">
